...
for it in range(max_iterations):    
    ...
    with etaT.dat.vec as evec:
        ...
        # Compute sum etaT
    with etaT_manual.dat.vec as evec:
        evec.abs()    
        etaT_manual_array = evec.getArray()
    
    etaT_total = (np.sum(etaT_array))
    etaT_manual_total = (np.sum(etaT_manual_array)) 
    etaTsum_vec.append(etaT_total)
    print(f"sum_T(eta_T) - automatic: {etaT_total}")
    print(f"sum_T(eta_T) - manual: {etaT_manual_total}")
